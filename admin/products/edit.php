<div class="bg-white p-5 min-h-[100vh] flex items-center">
  <form name="form" onsubmit="return validate()" class="w-full  mx-auto p-5 rounded-md  border" enctype="multipart/form-data" action="index.php?edit&id=<?= $row['id'] ?? "" ?>" method="post">
    ...
    <h2 class="text-xl text-center my-5">Sửa đổi sản phẩm</h2>
    ...
    <div class="flex flex-wrap -mx-3 mb-6">
      <div class="w-full px-3">
        ...
        <input value="<?= $row['name'] ?? '' ?>" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="name" type="text" name="name" placeholder="">
    ...
    <div class="flex flex-wrap -mx-3 mb-6 space-x-3">
      ...
      <div class="form-check space-x-1">
        <input value="1" class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="radio" name="active" id="active" <?= $row['active'] == 1 ? 'checked' : "" ?>>
        <label class="form-check-label inline-block text-gray-800" for="active">
          bán
        </label>
      </div>
      <div class="form-check space-x-1">
        <input value="0" class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="radio" name="active" id="active_not" <?= $row['active'] == 0 ? 'checked' : "" ?>>
    ...
    <div class="flex flex-wrap -mx-3 mb-2">
      <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
        ...
        <input value="<?= $row['price'] ?? "" ?>" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="" type="number" name="price" placeholder="">
      </div>
      <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
        <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-state">
          mã giảm giá
          ...
          <?php else : ?>
            <select name="voucher_id" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
              ...
              <?php foreach ($rows_voucher as $voucher) : ?>
                <option <?= $voucher['id'] == $row['voucher_id'] ? 'selected' : "" ?> value="<?= $voucher['id'] ?>">
    ...
    <div class="flex flex-wrap -mx-3 mb-6">
      <div class="w-full px-3">
        ...
        <textarea class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="" type="text" name="description" placeholder=""><?= $row['description'] ?? "" ?></textarea>
      </div>
    </div>
